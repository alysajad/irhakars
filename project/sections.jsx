        ...
        <div className="pkg-grid">
          {IRHA.PACKAGES.map(p => (
            <div className={`pkg reveal ${p.featured ? 'featured' : ''}`} key={p.name}>
              ...
              <button
                className={`cta-btn pick ${p.featured ? '' : 'outline'}`}
...
function Testimonials() {
  useReveal();
  const bgColors = ['#a0c4ff', '#ffadad', '#caffbf', '#bdb2ff', '#fdffb6', '#ffd6a5'];
  return (
    <section className="section" id="testimonials" style={{ background: 'var(--sand-2)' }}>
      <div className="wrap">
        <div className="s-head reveal" style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', textAlign: 'center', marginBottom: 48, justifyContent: 'center' }}>
          <div style={{ display: 'flex', alignItems: 'center', gap: 14, justifyContent: 'center' }}>
            ...
            <h2 className="display s-title" style={{ marginTop: 0, color: 'var(--teal-deep)', fontSize: 44, fontWeight: 500 }}>Customer Voices</h2>
          </div>
          <div style={{ width: 120, height: 2, background: 'var(--sand)', marginTop: 16, marginBottom: 16 }}></div>
          <p style={{ margin: 0, color: 'var(--ink-3)', fontSize: 16 }}>Real reviews from our travelers experiencing the spiritual and scenic beauty with us.</p>
          <div style={{ marginTop: 14, display: 'flex', alignItems: 'center', gap: 10, justifyContent: 'center' }}>
            <span style={{ color: '#fbbc04', fontSize: 24, fontWeight: '600' }}>4.8</span>
            <span style={{ color: '#fbbc04', letterSpacing: '2px', fontSize: 22 }}>★★★★★</span>
            <span style={{ color: 'var(--ink-3)', fontSize: 13, fontWeight: '600' }}>(120+ Reviews)</span>
          </div>
        </div>
        <div className="testimonials">
          {IRHA.TESTIMONIALS.map((t, i) => {
            const avaBg = bgColors[t.who.charCodeAt(0) % bgColors.length];
            return (
              <div className="t-card reveal" key={i} style={{ flex: '0 0 360px', scrollSnapAlign: 'start', position: 'relative', padding: '26px 26px 48px 26px', background: '#fff', borderRadius: 'var(--radius-lg)', boxShadow: 'var(--shadow-soft)', display: 'flex', flexDirection: 'column', gap: 16 }}>
                <div style={{ display: 'flex', alignItems: 'center', gap: 14 }}>
                  <div className="tava" style={{ width: 44, height: 44, borderRadius: '50%', background: avaBg, color: 'var(--ink)', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: 18, fontWeight: 700 }}>{t.ini}</div>
                  <div>
                    <div style={{ fontWeight: 700, fontSize: 15, color: 'var(--ink)', fontFamily: "'Open Sans', sans-serif" }}>{t.who}</div>
                    <div style={{ display: 'flex', alignItems: 'center', gap: 6, fontSize: 11, color: 'var(--ink-3)', marginTop: 2, fontWeight: 600 }}>
                      <span style={{ color: '#fbbc04', letterSpacing: '1px' }}>★★★★★</span>
                      <span>{t.time}</span>
                    </div>
                  </div>
                </div>
                <div className="tquote" style={{ fontSize: 14, color: 'var(--ink-2)', fontStyle: 'italic', lineHeight: 1.6, marginTop: 2 }}>"{t.quote}"</div>
                <svg viewBox="0 0 24 24" width="16" height="16" style={{ position: 'absolute', bottom: 20, right: 20, opacity: 0.7 }}>
                  <path fill="#4285F4" d="M22.56 12.25c0-.78-.07-1.53-.2-2.25H12v4.26h5.92c-.26 1.37-1.04 2.53-2.21 3.31v2.77h3.57c2.08-1.92 3.28-4.74 3.28-8.09z" />
                  <path fill="#34A853" d="M12 23c2.97 0 5.46-.98 7.28-2.66l-3.57-2.77c-.98.66-2.23 1.06-3.71 1.06-2.86 0-5.29-1.93-6.16-4.53H2.18v2.84C3.99 20.53 7.7 23 12 23z" />
                  <path fill="#FBBC05" d="M5.84 14.09c-.22-.66-.35-1.36-.35-2.09s.13-1.43.35-2.09V7.07H2.18C1.43 8.55 1 10.22 1 12s.43 3.45 1.18 4.93l2.85-2.22.81-.62z" />
                  <path fill="#EA4335" d="M12 5.38c1.62 0 3.06.56 4.21 1.64l3.15-3.15C17.45 2.09 14.97 1 12 1 7.7 1 3.99 3.47 2.18 7.07l3.66 2.84c.87-2.6 3.3-4.53 6.16-4.53z" />
